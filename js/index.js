...
var qs_path = '.'; 
var qs_mcmTheater_jar = "jar/mcmTheater-saverbeans-api.jar"; 
...
qs_menu_target[qs_location("MCM Basics", 'Speed')] = "basics/moveSpeed.html"; 
...
qs_menu_target[qs_location("Games", "Krtek")] = "games/krtek/krtek.html"; 
qs_menu_target[qs_location("Games", "TSP - Optimal")] = "games/tsp/tspOnePlayer.html"; 
qs_menu_target[qs_location("Games", "TSP - Better Of 2")] = "games/tsp/tspTwoPlayers.html"; 
...
qs_menu_image[qs_location("MCM Basics", 'Speed')] = "wheel"; 
...
qs_menu_image[qs_location("Games", "Krtek")] = "bubbles"; 
qs_menu_image[qs_location("Games", "TSP - Optimal")] = "bubbles"; 
qs_menu_image[qs_location("Games", "TSP - Better Of 2")] = "bubbles"; 
...
function qs_build_menu_table(menu_loc, sub_menu_loc) 
{
   ...
   if (qs_menu_item(menu_loc, sub_menu_loc, 'MCM Basics', undefined)) {
        ...
        qs_menu_item(menu_loc, sub_menu_loc, 'MCM Basics', 'Speed');
   ...
   if (qs_menu_item(menu_loc, sub_menu_loc, 'Games', undefined)) {
      qs_menu_item(menu_loc, sub_menu_loc, 'Games', 'Krtek'); 
      qs_menu_item(menu_loc, sub_menu_loc, 'Games', 'TSP - Optimal'); 
      qs_menu_item(menu_loc, sub_menu_loc, 'Games', 'TSP - Better Of 2'); 
   }
   qs_menu_separator(qs_color_img)
...
function qs_doc_finish(date, spacing) 
{
  ...
  wd('<script src="http://www.google-analytics.com/urchin.js" type="text/javascript">'); 
  wd('</script>'); 
  wd('<script type="text/javascript">'); 
  wd('_uacct = "UA-1493928-3";'); 
  wd('urchinTracker();'); 
  wd('</script>'); 
  wd('</body>');
...
function qs_create_applet(cls, archive, width, height) 
{
   var applet_name = 'applet_' + qs_applet_id; 
   qs_applet_id++;
...
{
   var applet = qs_create_applet(cls, archive, width, height); 
   var i;
   if (wincolor == undefined) 
      wincolor =  qs_applet_color;
   if (framecolor == undefined) 
      framecolor =  qs_applet_frame_color;
   applet.addParam("BG", wincolor); 
   if (params != undefined) { 
      for (i = 0; i < params.length; i++) {
         applet.addParam(params[i][0], params[i][1]); 
      }
   }

   s = qs_colored_table_start(3, framecolor, width); 
   s += '<TR><TD>\n';
   s += applet.getObjectString(); 
   s += '</TD></TR>';  
   s += qs_table_finish(); 
   applet.html = s;
   return applet;
...
{
      var applet = qs_applet(applet, archive, width, height, params, 
                         framecolor, wincolor); 
...
{
   var archive = pkg; 
   var applet_pkg  = pkg + "." + cls ;
   var javaSrc; 
   if (withSrcRef) {
      javaSrc = pkg + "/" + cls  + ".java";
   }
   wd(qs_applet_src(applet_pkg, javaSrc, archive, 
...
Applet.prototype.getHtmlString = function() {
   if (this.html == undefined) { 
      this.html = this.getObjectString();
   }
   return this.html;
}

Applet.prototype.geAppletHtml = function() {
   s = ""; 
...
Applet.prototype.geEmbedHtml = function() {
   s = ""; 
...
Applet.prototype.getObjectHtml = function() {
   s = '<OBJECT name="' + this._name + ' "classid="clsid:8AD9C840-044E-11D1-B3E9-00805F499D93"\n'; 
   s += '       width="' + this._width + '" ' ; 
   s += '       height="' + this._height + '" '; 
   s += '   ' + this._htmlTags + '\n'; 
   s+= '        codebase="http://java.sun.com">\n';
   s += '<PARAM NAME="code" VALUE="' + this._code + '">\n'; 
   s += '<PARAM NAME="archive" VALUE="' + this._archives + '">\n'; 
